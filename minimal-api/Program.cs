...
builder.Services.AddDbContext<MinimalApiContext>(x => x.UseSqlServer(connectionString));
...
{
    mc.AddProfile(new MinimalApiMapper(protector));
});
...
app.MapGet("/GetTop5UserPermisions", async (MinimalApiContext context, IZipService service) =>
{
...
app.MapPost("/InsertUser", async (MinimalApiContext context, User user, IMapper mapper) =>
{
...
app.MapGet("/GetAllUsersByName/{name}", [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)] async (HttpContext http, MinimalApiContext context, string name) =>
{
...
app.MapPost("/login", [AllowAnonymous] async (MinimalApiContext _context, HttpContext http, ITokenService service, Login login) =>
{
